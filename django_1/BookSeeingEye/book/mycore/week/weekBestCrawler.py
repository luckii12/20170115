...
def getDriver():
    try:
        driver = webdriver.Chrome(
            '/Users/canine/DEV/20170114/django_1/BookSeeingEye/book/driver/chromedriver', chrome_options=options)
    except:
        driver = webdriver.Chrome(
...
def pageCrawl(page, driver):
    if(driver == None):
        ...
        for i in range(1, 21):
            ...
            try:
                temp['ISBN'] = driver.find_element_by_css_selector(
                    '#tblGoodsFairTraderNoti tbody tr:nth-child(3) td').text
            except:
                temp['ISBN'] = 0

            temp['Publisher'] = driver.find_element_by_css_selector(
                'span.gd_pub a').text
            temp['Price'] = int(re.sub('[^0-9]', '', driver.find_element_by_xpath(
                '//*[@id="yDetailTopWrap"]/div[2]/div[2]/div[1]/div[1]/table/tbody/tr[1]/td/span/em').text))
            ...
            try:
                temp['SellingPoint'] = int(re.sub(
                    '[^0-9]', '', driver.find_element_by_css_selector('span.gd_sellNum').text))
            except:
                temp['SellingPoint'] = 'SP_IS_NONE'

            try:
                temp['Category'] = driver.find_element_by_css_selector(
                    '.basicListType ul').text.replace("\n", "<br/>")
                print(temp['Category'])
        ...
        try:
            if(item.book.isbn == yesterday_list.get(book=item.book).book.isbn):
                if(item.rank == yesterday_list.get(book=item.book).rank):
                    ...
                    riseandfall = yesterday_list.get(
                        book=item.book).rank - item.rank
